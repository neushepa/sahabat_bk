<div class="main-sidebar">
        <aside id="sidebar-wrapper">
          <div class="sidebar-brand">
            <a href="/dashboard">FATHFORCE</a>
          </div>
          <div class="sidebar-brand sidebar-brand-sm">
            <a href="index.html">FFSK</a>
          </div>
          <div class="mt-4 mb-4 p-3 hide-sidebar-mini">
            <a href="" class="btn btn-primary btn-lg btn-block btn-icon-split">
                <i class="fas fa-rocket"></i> Dashboard
            </a>
                <ul class="sidebar-menu">
                    <li><a class="nav-link" href="/todo"><i class="fas fa-calendar"></i> <span>To Do</span></a></li>
                    <li>
                            <li class="dropdown active">
                            <a href="#" class="nav-link has-dropdown"><i class="fas fa-th-large"></i> <span>Article</span></a>
                            <ul class="dropdown-menu" style="display: none;">
                                <li class="active"><a class="nav-link" href="/post">Post</a></li>
                                <li class="active"><a class="nav-link" href="/category">Category</a></li>
                            </ul>
                            </li>
                    </li>
                    <li><a class="nav-link" href="/gallery"><i class="fas fa-image"></i> <span>Gallery</span></a></li>
                    <li>
                            <li class="dropdown active">
                            <a href="#" class="nav-link has-dropdown"><i class="fas fa-th-large"></i> <span>User</span></a>
                            <ul class="dropdown-menu" style="display: none;">
                                <li class="active"><a class="nav-link" href="/backup">Guru</a></li>
                                <li class="active"><a class="nav-link" href="/import">Siswa</a></li>
                            </ul>
                            </li>
                    </li>
                    <li>
                            <li class="dropdown active">
                            <a href="#" class="nav-link has-dropdown"><i class="fas fa-th-large"></i> <span>Konsultasi</span></a>
                            <ul class="dropdown-menu" style="display: none;">
                                <li class="active"><a class="nav-link" href="/backup">Kelola Jadwal</a></li>
                                <li class="active"><a class="nav-link" href="/import">Kelola Konsultasi</a></li>
                                <li class="active"><a class="nav-link" href="/import">Forum Diskusi</a></li>
                            </ul>
                            </li>
                    </li>
                    <li>
                            <li class="dropdown active">
                            <a href="#" class="nav-link has-dropdown"><i class="fas fa-th-large"></i> <span>Settings</span></a>
                            <ul class="dropdown-menu" style="display: none;">
                                <li class="active"><a class="nav-link" href="/backup">Kelola Profile</a></li>
                                <li class="active"><a class="nav-link" href="/backup">Backup</a></li>
                                <li class="active"><a class="nav-link" href="/import">Import</a></li>
                                <li class="active"><a class="nav-link" href="/api">Change Password</a></li>
                            </ul>
                            </li>
                    </li>
                    <li><a class="nav-link" href="/credit"><i class="fas fa-fire"></i> <span>Credits</span></a></li>
                </ul>
        </aside>
      </div>
